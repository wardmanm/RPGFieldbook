...
function openItemForm(existing){
  ...
  const w=it.weapon||{};
  /* itemArmor(), not it.armor: pack armor carries no structured field at all —
     all 31 entries state their AC in the DESCRIPTION ("AC 15 + Dex modifier
     (max 2)"), which itemArmor() parses. Prefilling from the parsed value is
     what makes the hidden formula visible, and saving turns it into the
     structured field, which itemArmor() prefers from then on. */
  const arm=itemArmor(it);
  const armKind=arm?(arm.kind==="shield"?"shield":armorKindOf(arm)):"light";
  const armDex=arm&&arm.kind==="body"?(arm.dexCap==null?"":String(arm.dexCap)):"";
  ...
  const aKind=document.getElementById("iAKind"), aDexW=document.getElementById("iADexWrap"),
        aBaseW=document.getElementById("iABaseWrap"), aBonusW=document.getElementById("iABonusWrap");
  /* A shield is a flat bonus, not a base + Dex, so it swaps which boxes exist
     rather than showing three that do not apply to it. */
  function armSync(setDex){
    const k=aKind.value, sh=k==="shield";
    aBaseW.style.display=sh?"none":"";
    aBonusW.style.display=sh?"":"none";
    aDexW.style.display=sh?"none":"";
    if(setDex&&!sh){const d=ARMOR_DEXCAP[k];document.getElementById("iADex").value=(d==null?"":String(d));}
  }
  atog.addEventListener("click",()=>{isArmor=!isArmor;atog.classList.toggle("on",isArmor);afields.style.display=isArmor?"":"none";});
  aKind.addEventListener("change",()=>armSync(true));
  armSync(false);
  ...
  if(libSel)libSel.addEventListener("change",()=>{const x=lib[libSel.value];if(!x)return;document.getElementById("iName").value=x.name||"";document.getElementById("iDesc").value=x.description||"";const cg=costToGp(x.cost);if(cg!=null)document.getElementById("iCost").value=cg;const wg=fnum(x.weight);if(wg)document.getElementById("iWeight").value=wg;fxWrap.innerHTML=fxEditorRows(x.effects);
    ...
    isArmor=!!ax;atog.classList.toggle("on",isArmor);afields.style.display=isArmor?"":"none";
    if(ax){
      aKind.value=ax.kind==="shield"?"shield":armorKindOf(ax);
      armSync(false);
      if(ax.kind==="shield")document.getElementById("iABonus").value=num(ax.bonus||2);
      else{document.getElementById("iABase").value=num(ax.base);
           document.getElementById("iADex").value=ax.dexCap==null?"":String(ax.dexCap);}
    }
  ...
  document.getElementById("iSave").addEventListener("click",()=>{
    ...
    /* The structured field is what itemArmor() prefers, so saving here is what
       frees the item from depending on its description wording. Carried when the
       toggle is off so an untouched item keeps what it had — the same drop that
       cost attacks their itemId. */
    if(isArmor){
      const k=aKind.value;
      if(k==="shield"){rec.armor={kind:"shield",bonus:Math.max(0,num(document.getElementById("iABonus").value)||0)};}
      else{
        const raw=document.getElementById("iADex").value.trim();
        /* blank means "as the kind says"; "none" and 0 both mean no Dex at all */
        const dexCap=raw===""?ARMOR_DEXCAP[k]:(/^(none|-)$/i.test(raw)?0:Math.max(0,num(raw)));
